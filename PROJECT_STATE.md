...
- 2026-07-19: Added weekly plan evidence metadata to `/plan-health` and the rendered header (`LOW/MODERATE/HIGH CONFIDENCE · phase`). Replaced the pace-derived VO₂max estimate with the latest measured Apple Health `vo2max.csv` value and source date; missing Apple Health VO₂max now renders unavailable rather than inventing an estimate. Six tests pass. Source was pushed to GitHub `main`; live NAS rebuild remains blocked because SSH sudo requires an interactive password.
