...
<style>
  ...
  * {{ margin:0; padding:0; box-sizing:border-box; }}
  body {{ width:800px; height:480px; overflow:hidden; background:#fff; color:#000; font-family:'IBM Plex Mono',monospace; font-size:11px; }}
  ...
  .strava-mark {{ font-size:10px; font-weight:700; letter-spacing:0.15em; display:flex; align-items:center; gap:5px; }}
  .header-title {{ font-size:10px; font-weight:600; letter-spacing:0.04em; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }}
  .header-phase {{ font-size:8px; letter-spacing:0.1em; border:1px solid #fff; padding:1px 7px; text-transform:uppercase; white-space:nowrap; }}
  .header-date {{ font-size:9px; letter-spacing:0.08em; opacity:0.7; white-space:nowrap; }}
  ...
  .today-eyebrow {{ font-size:7.5px; letter-spacing:0.2em; text-transform:uppercase; color:#666; margin-bottom:3px; }}
  .today-title {{ font-size:21px; font-weight:700; line-height:1.05; letter-spacing:-0.02em; font-family:'IBM Plex Sans',sans-serif; margin-bottom:5px; }}
  .today-detail {{ font-size:9.5px; color:#333; line-height:1.55; }}
  .today-tags {{ display:flex; gap:5px; margin-top:7px; flex-wrap:wrap; }}
  .tag {{ font-size:7.5px; font-weight:700; letter-spacing:0.1em; text-transform:uppercase; border:1.5px solid #000; padding:2px 6px; }}
  .tag-filled {{ background:#000; color:#fff; }}
  .tag-outline {{ background:#fff; color:#000; }}
  .hr-alert {{ display:flex; align-items:center; gap:5px; margin-top:7px; font-size:9.5px; font-weight:700; }}
  .week-strip {{ padding:7px 13px; display:flex; flex-direction:column; justify-content:center; gap:4px; }}
  .week-strip-label {{ font-size:7.5px; letter-spacing:0.14em; text-transform:uppercase; color:#888; }}
  ...
  .wd-name {{ font-size:6.5px; font-weight:700; letter-spacing:0.06em; color:#777; }}
  .wd-box {{ width:30px; height:30px; border:1.5px solid #bbb; display:flex; align-items:center; justify-content:center; font-size:14px; background:#fff; }}
  .wd-today {{ border-color:#000; border-width:2.5px; }}
  .wd-done {{ border-color:#000; background:#000; color:#fff; font-size:12px; }}
  .wd-rest {{ border-color:#ddd; background:#f5f5f5; font-size:9px; color:#999; }}
  .wd-dur {{ font-size:6px; color:#999; text-align:center; }}
  .col-mid {{ padding:11px 13px; display:flex; flex-direction:column; }}
  .col-label {{ font-size:7.5px; letter-spacing:0.18em; text-transform:uppercase; color:#777; margin-bottom:8px; display:flex; align-items:center; gap:6px; }}
  ...
  .ss-label {{ font-size:7.5px; letter-spacing:0.1em; text-transform:uppercase; color:#999; margin-bottom:1px; }}
  .ss-value {{ font-size:18px; font-weight:700; letter-spacing:-0.02em; font-family:'IBM Plex Sans',sans-serif; line-height:1; }}
  .ss-sub {{ font-size:7.5px; color:#777; margin-top:1px; }}
  ...
  .vo2-unit {{ font-size:8px; color:#777; margin-left:3px; }}
  ...
  .bar-labels {{ display:flex; justify-content:space-between; margin-top:3px; font-size:7px; color:#999; }}
  ...
  .phase-meta {{ display:flex; justify-content:space-between; margin-top:3px; font-size:7px; color:#999; }}
  .recent-item {{ display:flex; align-items:center; justify-content:space-between; padding:4px 0; border-bottom:1px solid #efefef; font-size:8.5px; }}
  .recent-item:last-child {{ border-bottom:none; }}
  .ri-left {{ display:flex; align-items:center; gap:5px; }}
  .ri-icon {{ font-size:11px; }}
  .ri-name {{ font-weight:600; font-size:8.5px; }}
  .ri-sub {{ font-size:7px; color:#999; }}
  .ri-right {{ text-align:right; }}
  .ri-dist {{ font-weight:700; font-size:9px; }}
  .ri-time {{ font-size:7px; color:#999; }}
  ...
  .fc-label {{ font-size:7px; text-transform:uppercase; letter-spacing:0.1em; color:#888; margin-bottom:1px; }}
  .fc-val {{ font-size:11px; font-weight:700; font-family:'IBM Plex Sans',sans-serif; line-height:1.1; }}
  .fc-sub {{ font-size:7px; color:#777; }}
</style>
</head>
<body>
<div class="screen">
